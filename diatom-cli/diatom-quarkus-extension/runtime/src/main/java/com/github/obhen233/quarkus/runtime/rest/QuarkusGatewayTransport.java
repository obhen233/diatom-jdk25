...
/**
 * 基于 JDK {@link HttpURLConnection} 的 Gateway 传输层（框架无关）。
 *
 * <p>拷贝自 starter {@code SpringGatewayTransport}（原名，框架无关）：
 * 用于 Gateway → Worker 的 HTTP 通信。以 WorkerRegistry 解析目标 URL，
 * 携带 HTTP 状态码供 Gateway 侧识别 503/429（Worker 过载）并换 worker 重试。</p>
 */
public class QuarkusGatewayTransport {
    private static final Logger LOGGER = Logger.getLogger(QuarkusGatewayTransport.class);

    private final WorkerRegistry registry;

    public QuarkusGatewayTransport(WorkerRegistry registry) {
    ...
    public HttpResult sendChatRequestResult(String workerId, String requestBody) {
        ...
        if (worker == null) {
            LOGGER.warnf("Worker not found: %s", workerId);
    ...
    private HttpResult httpPostResult(String urlStr, String body) {
        ...
        try {
            ...
            LOGGER.warnf("HTTP POST failed to %s: %s", urlStr, e.getMessage());
            return new HttpResult(-1, null);
        } finally {
            if (conn != null) {
                conn.disconnect();
